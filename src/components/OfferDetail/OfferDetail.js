...
export default ({ data, proposals, vouches, className, type, isMyOffer }) => {
  const statusClass = OFFER_STATUS_CLASS[data.status.toLowerCase()];
  ...
  return (
    <div className={`offer-detail ${className}`}>
      <Row className="mx-0 mb-2 px-4 d-flex flex-row justify-content-between align-items-center">
        ...
        <div
          className={`offer-status border border-${statusClass} text-${statusClass} rounded px-2 font-weight-bold text-uppercase p-sm text-center`}
        >
          {data.status}
        </div>
      </Row>
      <Row className="mx-0 px-4 mb-3 d-flex flex-row justify-content-between p-sm align-items-center">
        <div className="d-flex flex-row align-item-center">
          by {data.offeredBy.firstName} {data.offeredBy.lastName}{' '}
          <Rating
            className="ml-2"
            initialRating={data.offeredBy.avgRating}
            readonly
          />
        </div>
